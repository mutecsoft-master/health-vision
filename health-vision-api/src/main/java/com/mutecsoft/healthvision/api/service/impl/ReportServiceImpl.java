...
import com.mutecsoft.healthvision.common.mapper.ReportMapper;
...
public class ReportServiceImpl implements ReportService {
	...
	@Override
	public ResponseDto applyReportForGoogle(ReportApplyGoogleRequest applyReq) {
		
		ProductPurchase verifyPurchase = googleIapService.verifyPurchase(applyReq.getPurchase());
		...
		if(verifyPurchase.getPurchaseState() == Integer.parseInt(GooglePurchaseStatusEnum.PURCHASED.getValue())
			...
			ph.setPurchaseToken(applyReq.getPurchase().getPurchaseToken());
			ph.setProductId(applyReq.getPurchase().getProductId());
			...
			try {
				...
				InAppProduct productInfo = googleIapService.getProductInfo(applyReq.getPurchase());
			...
			Report report = null;
			if(verifyPurchase.getPurchaseState() == Integer.parseInt(GooglePurchaseStatusEnum.PURCHASED.getValue())) {
				report = applyReport(applyReq.getBgFileId(), ReportStatusCdEnum.APPLY);
			}else {
				//일단 대기로 저장하고, 이후 google에서 오는 알림을 이용해 처리
				report = applyReport(applyReq.getBgFileId(), ReportStatusCdEnum.PENDING);
			}
	...
	@Override
	public ResponseDto applyReportForApple(ReportApplyAppleRequest applyReq) {
		
		try {
			Map<String, String> verifyMap = appleIapService.verifyReceipt(applyReq.getPurchase());
			...
			if(!verifyMap.isEmpty()) {
				...
				//애플은 소모성 구매에 대해 별도 알림을 제공하지 않으므로 바로 '신청'상태 적용
				Report report = applyReport(applyReq.getBgFileId(), ReportStatusCdEnum.APPLY);
	...
	@Override
	public ResponseDto applyReportForFree(ReportApplyFreeRequest applyReq) {
		applyReport(applyReq.getBgFileId(), ReportStatusCdEnum.APPLY);
		return new ResponseDto(true);
	}
	
	private Report applyReport(Long bgFileId, ReportStatusCdEnum statusCdEnum) {
		
		UserInfo userInfo = userUtil.getUserInfo();
		
		BgFile bgFile = bgFileService.getBgFileById(bgFileId);
		if(bgFile == null) {
		...
		ReportResponse registeredReport = reportMapper.selectReportByBgFileId(bgFileId);
		...
		report.setStatusCd(statusCdEnum.getValue());
